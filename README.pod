...
This is the main call to the library for I<sparse> Jacobians. Its declared as
...
=item * If not C<NULL>, C<returnContext> can be used to retrieve the full context structure from the
solver. This can be useful since this structure contains the latest operating point values. I<If
this data is requested, the user is required to free it with C<dogleg_freeContext> when done>.
