...
C<libdogleg> is not deprecated. I use it heavily, and I do add features and fix
bugs. However C<ceres> is more feature-rich and I<much> more widely used. I
consider C<ceres> to be a superset of C<libdogleg>, and if C<ceres> was
available when I wrote C<libdogleg>, I would not have written it.
...
libdogleg is available in non-ancient Debian (and Ubuntu and all the downstream
distros): simply

 apt install libdogleg-dev

If you want mote recent packages than what your distro provides, builds are
available in the L<mrcal APT repo|https://mrcal.secretsauce.net/install.html>.

If you want to help get libdogleg into more distributions, send me an email. It
can be built and used in many places that lack packages (Fedora, FreeBSD, macos,
...), and making a package shouldn't be a lot of work.

To build from source, you need the dependencies in the Build-Depends section of
the L<Debian package definition|https://salsa.debian.org/science-team/libdogleg/-/blob/master/debian/control>

When those are installed, simply run
